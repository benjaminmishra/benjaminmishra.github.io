...
const ExperienceEntry: React.FC<ExperienceEntryProps> = ({ experience, summary, isSelected, onClick }) => {
  return (
    <div
      className={`relative border-l-2 border-blue-500 pl-8 cursor-pointer pr-4 mb-4 transition-all duration-300 overflow-hidden ${isSelected ? 'border-blue-700 bg-blue-100 bg-opacity-75 scale-105 shadow-lg pb-12 min-h-[300px]' : 'border-blue-500 bg-white pb-4 min-h-[150px]'}`}
      onClick={onClick}
    >
      ...
      <p className="text-lg font-semibold text-gray-700">{experience.title}</p>
      <p className="text-sm text-gray-600">{experience.duration}</p>

      {isSelected && (
        <div className="mt-2 text-gray-700 overflow-auto">
